...
// Function to find all palindromic permutations of given string
void palindromicPermutations(string str)
{
	...
	for (auto itr: freq)
	{
		...
		if ((c & 1))			// if count of current character is odd
		{
			...
			// palindromic permutations is not possible
			if (++odd > 1)
				return;
	...
	// Note that we can also sort in reverse order and use 
